...
Additional implemented effects are `fire`, `rotating-plane`, `radar`, `aurora`, and `fireflies`. They all use the generated XYZ data rather than LED index order. Fire, aurora, and fireflies use `--duration`/`--hold`; rotating-plane and radar also expose meaningful `--loops`. `thunderdome effect auto` cycles continuously by default and supports `--interval`, `--crossfade`/`--transition`, `--playlist`/`--effects`, `--preset calm|energetic`, finite `--cycles` or `--duration`, and linear RGB crossfade with brightness applied once after blending.
