...
Additional implemented effects are `fire`, `rotating-plane`, `radar`, `aurora`, and `fireflies`. They all use the generated XYZ data rather than LED index order. Fire, aurora, and fireflies use `--duration`/`--hold`; rotating-plane and radar also expose meaningful `--loops`. `rotating-plane --axis` is a real 3D rotation axis (`vertical=(0,0,1)`, `horizontal=(1,0,0)`, `tilted=normalize(1,1,1)`, or explicit `X,Y,Z`) and `--trail-degrees 0` disables its directional fading trail. `thunderdome effect auto` cycles continuously by default and supports `--interval`, `--crossfade`/`--transition`, `--playlist`/`--effects`, `--preset calm|energetic`, finite `--cycles` or `--duration`, and linear RGB crossfade with brightness applied once after blending while preserving incoming effect time across transition boundaries.
