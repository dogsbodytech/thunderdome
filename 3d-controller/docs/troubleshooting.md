...
| --- | --- | --- | --- |
| `thunderdome` command not found | venv is not active; package not installed | Run `source .venv/bin/activate` and `thunderdome --help` | [Rebuild/install](setup-and-recovery/controller-host.md) |
| Venv problems | Python older than 3.11; venv package missing; broken environment | Run `python3 --version` and `test -x .venv/bin/python` | Recreate `.venv` and follow [software setup](runbooks/01-software-and-simulator.md) |
...
| Browser cannot open simulator | Server stopped; wrong URL/port; browser local access issue | Confirm the server printed `http://127.0.0.1:8080/` and is still running | Open the printed URL or use `--port`; see [simulator reference](technical/simulator.md) |
| Simulator loads but no live movement | Effect not started; wrong simulator URL; producer connection failed | Run a finite effect with `--output simulator` and check the CLI output mode | Check `/ws/producer` and [software runbook](runbooks/01-software-and-simulator.md) |
| Simulator works but physical dome is dark | No power; WLED off; wrong host; realtime/DDP disabled; wiring/fuse issue | Read WLED state and check controller reachability | Follow [physical startup](runbooks/02-physical-dome-startup.md) and [first light](runbooks/03-first-light-and-ddp.md) |
| Controller config invalid | Placeholder host; wrong number/string/range; malformed JSON | Run `thunderdome controllers validate --controllers config/controllers.json` | Recreate from `config/controllers.example.json`; see [WLED Controller recovery](setup-and-recovery/wled-controller.md#controllersjson) |
...
| WLED reachable but DDP appears ignored | DDP/realtime disabled; wrong UDP port; LED count/output mismatch; WLED state off | Confirm UDP/4048, LED count/output settings, realtime state, and power | Compare the device with [WLED Controller recovery](setup-and-recovery/wled-controller.md), then repeat first light |
...
| Geometry looks wrong | Wrong geometry/routes/positions set; camera view; stale generated data | Run geometry/route/position validation and reset the simulator view | Regenerate positions and use [geometry](technical/geometry.md); do not reshape coordinates in the browser |
| Control UI unavailable | Control service not running; port occupied; service is simulator-only; browser URL wrong | Check the service terminal and `curl -s http://127.0.0.1:8080/api/control/capabilities` | Follow [control service](technical/control-service.md) and [REST reference](technical/api-rest.md) |
...
| Factory-reset/replacement WLED controller | Address, LED count, output, realtime, or power/current settings lost | Compare the device with its matching public export | Follow [WLED Controller recovery](setup-and-recovery/wled-controller.md), then first light |

## Return to the numbered sequence

Troubleshooting is not a parallel operating path. After fixing a fault, return to the checkpoint that proves the affected layer:

| Fixed problem | Resume at |
| --- | --- |
| Python, venv, geometry, routes, positions, simulator, or live browser movement | [01 — Software and simulator](runbooks/01-software-and-simulator.md) |
| Controller configuration, WLED reachability, network, or physical readiness | [02 — Physical dome startup](runbooks/02-physical-dome-startup.md) |
| Controller/string identity, first pixel, colours, or DDP mapping | [03 — First light and DDP](runbooks/03-first-light-and-ddp.md) |
| Confirmed known-good installation ready for an event | [04 — Normal operation](runbooks/04-normal-operation.md) |
| Any active software must be stopped | [05 — Shutdown](runbooks/05-shutdown.md) |
